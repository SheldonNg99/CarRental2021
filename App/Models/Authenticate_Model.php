...
class Authenticate_Model extends Dbh{
  ...
  public function UserRegistration($data){
    ...
    $sql = "INSERT INTO Users(User_Name,User_Email,User_Password,User_Authorization_Level,User_Register_Time) VALUES (?,?,?,?,?)";
    $stmt = $this->connect()->prepare($sql);
    $stmt->execute([$username,$email,$Hash_password,$Type,$User_Register_Datetime]);
  ...
  //Check Authorizatin level
  public function UserAuthorizationLevel(){
    $username = $_SESSION['name'];

    $sql = "SELECT User_Authorization_Level FROM Users WHERE User_Name = ?";
    $stmt = $this->connect()->prepare($sql);
    $stmt->bindParam('s',$username);
    $stmt->execute([$username]);
    ...
    if($results > 0){
      foreach($results as $result){
        if ($result['User_Authorization_Level'] == 'Admin') {
          $results['DataError'] = array(
              'message' => "IsAdmin",
          );
          return $results;
